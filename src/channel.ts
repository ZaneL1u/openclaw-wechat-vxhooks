...
import { displayLoginSuccess } from './utils/qrcode.js'
...
export const wechatPlugin: ChannelPlugin<ResolvedWeChatAccount> = {
  ...
  directory: {
    ...
    listPeers: async ({ cfg, query, limit, accountId }) => {
      const account = resolveWeChatAccount({ cfg, accountId: accountId || DEFAULT_ACCOUNT_ID })
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // If not logged in, perform DLL login (no QR code needed)
